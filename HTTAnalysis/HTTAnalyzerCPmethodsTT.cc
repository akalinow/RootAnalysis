...
void HTTAnalyzerTT::fillDecayPlaneAngle(const std::string & hNameSuffix, float eventWeight){
  ...
  TLorentzVector tau1PCA(aTau1.getPCARefitPV(),0);
  TLorentzVector tau2PCA(aTau2.getPCARefitPV(),0);
...
void HTTAnalyzerTT::fillGenDecayPlaneAngle(const std::string & hNameSuffix, float eventWeight){
  ...
  if(aGenTau1.getCharge()>0) {
    angles = angleBetweenPlanes(tau1LeadingTk, tau1PCA, tau2LeadingTk, tau2PCA);
    anglesIPRhoNeg = angleBetweenPlanes(tau1LeadingTk, tau1PCA, tau2LeadingTk, aGenTau2.getNeutralP4());
    anglesIPRhoPos = angleBetweenPlanes(tau1LeadingTk, aGenTau1.getNeutralP4(), tau2LeadingTk, tau2PCA);
    anglesRhoRho = angleBetweenPlanes(tau1LeadingTk, aGenTau1.getNeutralP4(), tau2LeadingTk, aGenTau2.getNeutralP4());
  }
  else{
    angles = angleBetweenPlanes(tau2LeadingTk, tau2PCA, tau1LeadingTk, tau1PCA);
    anglesIPRhoNeg = angleBetweenPlanes(tau2LeadingTk, tau2PCA, tau1LeadingTk, aGenTau1.getNeutralP4());
    anglesIPRhoPos = angleBetweenPlanes(tau2LeadingTk, aGenTau2.getNeutralP4(), tau1LeadingTk, tau1PCA);
    anglesRhoRho = angleBetweenPlanes(tau2LeadingTk, aGenTau2.getNeutralP4(), tau1LeadingTk, aGenTau1.getNeutralP4());
  }
  ///////////////////////////////////////////////////////////                   
  /* pure gen definitions */
  bool isPi1 = aGenTau1.getProperty(PropertyEnum::decayMode)==tauDecay1ChargedPion0PiZero;
  ...
  bool isIP1 = (isOneProng(aGenTau1.getProperty(PropertyEnum::decayMode)) &&
                tau1PCA.Vect().Mag()>nPCAMin_);
  bool isIP2 = (isOneProng(aGenTau2.getProperty(PropertyEnum::decayMode)) &&
                tau2PCA.Vect().Mag()>nPCAMin_ );
  ...
  //1
  if( isPi1 && isRho2 ){
    float yTau =  2.*tau2LeadingTk.Pt()/aGenTau2.getP4().Pt() - 1.;
    float ipRho = anglesIPRhoNeg.first;
    std::string suffix = "Neg";
    if(aGenTau2.getCharge()>0){
      ipRho = anglesIPRhoPos.first;
      suffix = "Pos";
    }
    float shiftedIPRho = ipRho +(yTau<0)*(1.-2.*(ipRho>M_PI))*M_PI;
  ...
  //2
  if( isPi2 && isRho1 ){
    float yTau =  2.*tau1LeadingTk.Pt()/aGenTau1.getP4().Pt() - 1.;
    float ipRho = anglesIPRhoNeg.first;
    std::string suffix = "Neg";
    if(aGenTau1.getCharge()>0){
      ipRho = anglesIPRhoPos.first;
  ...
  if( isIP1 && isRho2 ){
    ...
    float shiftedIPRho = ipRho +(yTau<0)*(1.-2.*(ipRho>M_PI))*M_PI;
     ...
     myHistos_->fill1DHistogram("h1DyTau"+suffix+"_IP"+hNameSuffix,yTau,eventWeight);
     myHistos_->fill1DHistogram("h1DPhi_IPRho"+suffix+"_IP_"+hNameSuffix,shiftedIPRho,eventWeight);
     myHistos_->fill1DHistogram("h1DPhi_IPRho_IP_"+hNameSuffix,shiftedIPRho,eventWeight);
     if(yTau>0){
       myHistos_->fill1DHistogram("h1DPhi_IPRho"+suffix+"_yTauPos_IP"+hNameSuffix,ipRho,eventWeight);
       myHistos_->fill1DHistogram("h1DPhi_IPRho_yTauPos_IP"+hNameSuffix,ipRho,eventWeight);
     }
     else{
       myHistos_->fill1DHistogram("h1DPhi_IPRho"+suffix+"_yTauNeg_IP"+hNameSuffix,ipRho,eventWeight);
       myHistos_->fill1DHistogram("h1DPhi_IPRho_yTauNeg_IP"+hNameSuffix,ipRho,eventWeight);
     }
  }  
  //2
  if( isIP2 && isRho1 ){
  ...
  //Rho+Rho
  if(isRho1 && isRho2){
    float yTau1 =  2.*tau1LeadingTk.Pt()/aGenTau1.getP4().Pt() - 1.;
    float yTau2 =  2.*tau2LeadingTk.Pt()/aGenTau2.getP4().Pt() - 1.;
    float yTau12 = yTau1*yTau2; 
    float shiftedRhoRho = anglesRhoRho.first +(yTau12<0)*(1.-2.*(anglesRhoRho.first>M_PI))*M_PI;
     myHistos_->fill1DHistogram("h1DyTau1"+hNameSuffix,yTau1,eventWeight);
     myHistos_->fill1DHistogram("h1DyTau2"+hNameSuffix,yTau2,eventWeight);
