...
float HTTHistograms::getSampleNormalisation(std::string sampleName){
  ...
  if(sampleName=="VBFH"){
    ///mH = 125, VBF fussion only
    //https://twiki.cern.ch/twiki/pub/LHCPhysics/LHCHXSWG/Higgs_XSBR_YR4_update.xlsx
    //https://twiki.cern.ch/twiki/bin/view/LHCPhysics/CERNYellowReportPageBR3
    crossSection = 3.782*6.32E-02; 
  }
...
TH1F *HTTHistograms::get1D_DY_Histogram(const std::string& name){
  ...
  if(name.find("h1DStats")==std::string::npos){
    if(hDYJetsMuTau) hDYJetsMuTau->Scale(muTauDYScale);
    if(hDYJetsMuMu) hDYJetsMuMu->Scale(mumuDYScale);
  }
...
TH1F *HTTHistograms::getNormalised_NJet_Histogram(const std::string& hName,
  ...
  if(jetsName.find("0Jets")!=std::string::npos){
    TString allJetsName = "h1DStats"+jetsName;
    allJetsName.ReplaceAll("0Jets","AllJets");
    TH1F *hAllJetsStats = get1DHistogram(allJetsName.Data());
  ...
  hNJets->Scale(recoPresEff/nEventsAnalysed);
  return hNJets;
}
/////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////
TH1F *HTTHistograms::get1D_WJet_Histogram(const std::string& name){
  ...
  TH1F *hW0Jets = getNormalised_NJet_Histogram(hName.Data(),"W0Jets");

  hName = name;
  hName.ReplaceAll("WJets","W1Jets");
  TH1F *hW1Jets = getNormalised_NJet_Histogram(hName.Data(),"W1Jets");
    
  hName = name;
  hName.ReplaceAll("WJets","W2Jets");
  TH1F *hW2Jets = getNormalised_NJet_Histogram(hName.Data(),"W2Jets");

  hName = name;
  hName.ReplaceAll("WJets","W3Jets");
  TH1F *hW3Jets = getNormalised_NJet_Histogram(hName.Data(),"W3Jets");

  hName = name;
  hName.ReplaceAll("WJets","W4Jets");
  TH1F *hW4Jets = getNormalised_NJet_Histogram(hName.Data(),"W4Jets");

  TH1F *hWJets = 0;
  if(hW0Jets) hWJets = (TH1F*)hW0Jets->Clone(name.c_str());
  else if(hW1Jets) hWJets = (TH1F*)hW1Jets->Clone(name.c_str());
  else if(hW2Jets) hWJets = (TH1F*)hW2Jets->Clone(name.c_str());

  if(!hWJets) return 0;

  float wJetsLOSigma[5] = {50380, 9644.5, 3144.5, 954.8, 485.6};
  float zJetsLOSigma[5] = {4954.0, 1012.5, 332.8, 101.8, 54.8};
  
  hWJets->Reset();
  if(hW0Jets) hWJets->Add(hW0Jets, wJetsLOSigma[0]/wJetsLOSigma[0]);
  if(hW1Jets) hWJets->Add(hW1Jets, wJetsLOSigma[1]/wJetsLOSigma[0]);
  if(hW2Jets) hWJets->Add(hW2Jets, wJetsLOSigma[2]/wJetsLOSigma[0]);
  if(hW3Jets) hWJets->Add(hW3Jets, wJetsLOSigma[3]/wJetsLOSigma[0]);
  if(hW4Jets) hWJets->Add(hW4Jets, wJetsLOSigma[4]/wJetsLOSigma[0]);
...
void HTTHistograms::defineHistograms(){
 ...
 if(!histosInitialized_){
   ...
   add1DHistogram("h1DIsoTemplate",";Isolation; Events",10,0,0.3,file_);
...
void HTTHistograms::plotCPhistograms(int nRuns, float weight){
  ...
  plotPhiDecayPlanes("Phi_nVectorsggH");
  ...
  plotProfiles("hProfRecoVsMagGen_","ggH");
  plotProfiles("hProfRecoVsMagGen_","A");
  plotProfiles("hProfRecoVsMagGen_","DYJetsMuTau");

  plotProfiles("hProfPhiVsMag_","ggH");
  plotProfiles("hProfPhiVsMag_","A");
  plotProfiles("hProfPhiVsMag_","DYJetsMuTau");

  plotVerticesPulls("h1DVxPullX_ggH");
  plotVerticesPulls("h1DVxPullY_ggH");
  plotVerticesPulls("h1DVxPullZ_ggH");
  ...
  plotPhiDecayPlanes("Phi_nVecIP_yTauPosggH");
  plotPhiDecayPlanes("Phi_nVecIP_yTauNegggH");
...
void HTTHistograms::plot_HAZ_Histograms(const std::string & hName,
  ...
  TString name = "h1D"+hName+"ggH"+sysType;  
...
THStack*  HTTHistograms::plotStack(std::string varName, std::string selName){
  ...
  TH1F *hggHiggs = get1DHistogram((hName+"ggH"+selName).c_str());
  TH1F *hVBFHiggs = get1DHistogram((hName+"VBFH"+selName).c_str());
  ...
  if(!hDYJetsOther) hDYJetsOther = (TH1F*)hEmpty->Clone((hName+"hDYJetsOther"+selName).c_str());  
  if(!hDYJetsMuTau) hDYJetsMuTau = (TH1F*)hEmpty->Clone((hName+"hDYJetsMuTau"+selName).c_str());  
  if(!hDYJetsMuMu) hDYJetsMuMu = (TH1F*)hEmpty->Clone((hName+"hDYJetsMuMu"+selName).c_str());  
  if(!hDYJetsEE) hDYJetsEE = (TH1F*)hEmpty->Clone((hName+"hDYJetsEE"+selName).c_str());  
  if(!hTTbar) hTTbar = (TH1F*)hEmpty->Clone((hName+"hTTbar"+selName).c_str());  
  if(!hggHiggs) hggHiggs = (TH1F*)hEmpty->Clone((hName+"hggH"+selName).c_str());  
  if(!hVBFHiggs) hVBFHiggs = (TH1F*)hEmpty->Clone((hName+"hVBFH"+selName).c_str());  
  ...
  weight = getSampleNormalisation(sampleName);
  scale = weight*lumi;
  hggHiggs->Scale(scale);
  
  sampleName = "VBFH";
  weight = getSampleNormalisation(sampleName);
  scale = weight*lumi;
  hVBFHiggs->Scale(scale);

  TH1F *hHiggs = (TH1F*)hggHiggs->Clone("hHiggs");
  hggHiggs->Add(hVBFHiggs);
  //////////////////////////////////////////////////////
