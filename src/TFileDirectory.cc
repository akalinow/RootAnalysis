...
TFileDirectory::_cd (const string &subdir, bool createNeededDirectories) const
{
   ...
   {      
      ...
      if ( ! path_.empty() ) 
      {
         ...
         {
            throw 
               std::string( "InvalidDirectory Can't change directory to path: " +path_);
         }
      } else 
      {
   ...
   {
      throw 
         std::string("InvalidDirectory Can't change directory to path: "+fpath);
   }
   return dir;
}
                        ...
                        const string &description) const
{
   ...
   {
      throw 
         std::string("InvalidDirectory Can't create directory "+dir_+" in path: "+path_);
   }
   subdirPtr->cd();
   return subdirPtr;
}
...
TFileDirectory::_getObj (const string &objname, const string &subdir) const
{
   ...
   if ( ! objPtr)
   {
      ...
      {
         throw
            std::string("ObjectNotFound Can not find object named "+ objname+ " in subdir " +subdir);
      } else {
         throw
            std::string ("ObjectNotFound Can not find object named " + objname);
      }
   } // if nothing found
   return objPtr;
}
