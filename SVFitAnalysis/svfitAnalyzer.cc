...
TLorentzVector svfitAnalyzer::computeMTT(const std::string & algoName){
  ...
  //double cosGJGen = (genSV - genPV).Unit()*aP4.Vect().Unit();//TEST
  //ip3D = aGenLeg1.getPCA().Mag();//TEST
  //cosGJReco = cosGJGen; //TEST
...
std::tuple<double, double> svfitAnalyzer::getTauMomentum(const TLorentzVector & visP4, double cosGJ){
  ...
  std::cout<<" decayMode: "<<aGenLeg2.getProperty(PropertyEnum::decayMode)
	   ...
	   <<" cosGJ: "<<cosGJ
    	   <<" cosGJ2: "<<cosGJ2
    	   <<" sinGJ2: "<<sinGJ2
	   <<" (mVis/mTau)^2: "<<mVis2/mTau2
           <<" eMin: "<<tauEnergy1
	   <<" eMax: "<<tauEnergy2    
           <<" xMin: "<<solution1
	   <<" xMax: "<<solution2
	   <<" true value: "<<(aGenLeg2.getChargedP4() + aGenLeg2.getNeutralP4()).E()/aGenLeg2.getP4().E()
	   <<std::endl;
  */
...
void svfitAnalyzer::fillControlHistos(const std::string & hNameSuffix){
  ...
  myHistos_->fill1DHistogram("h1DDeltaMVisMSV"+hNameSuffix,aPair.getP4().M()/visMass);
  ...
  */
  if(isThreeProng){
    ...
    if(trueX2>1.0 || trueX1>1.0){
      std::cout<<" trueX1: "<<trueX1
	       <<" trueX2: "<<trueX2<<std::endl;
    }
...
bool svfitAnalyzer::analyze(const EventProxyBase& iEvent){
  ...
  goodGenTau &= aGenLeg2.getProperty(PropertyEnum::decayMode) == HTTAnalysis::hadronicTauDecayModes::tauDecay3ChargedPion0PiZero;
